...
#define VARIO_SPEED_LIMIT 10 //m/s
    ...
    if(verticalSpeed < g_model.frsky.varioSpeedUpMin*VARIO_LIM_MUL && verticalSpeed > g_model.frsky.varioSpeedDownMin*(-VARIO_LIM_MUL)) //check thresholds here in cm/s
    {
      SoundAltBeepNextFreq = (0);
      SoundAltBeepNextTime = (0);
    }else{
      SoundAltBeepNextFreq = (((verticalSpeed * 56) + 77400)/10)/10/30;
      ...
      if(verticalSpeed > 0){
        if ((int16_t)(g_tmr10ms - s_varioTmr) > (int16_t)(SoundAltBeepNextTime*2)) {
          ...
          audio.playNow(SoundAltBeepNextFreq, SoundAltBeepNextTime, 0, 0, 0);
        }	
      } else {//negative vertical speed gives sound without pauses
        if (g_tmr10ms - s_varioTmr > SoundAltBeepNextTime) {
          s_varioTmr = g_tmr10ms;
          audio.playNow(SoundAltBeepNextFreq, SoundAltBeepNextTime+2, 0, 0, 0);
        }	
      }			  
    }		  
  }
