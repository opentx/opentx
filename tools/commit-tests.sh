...
if [[ " SKY9X ARM9X ALL " =~ " ${FLAVOR} " ]] ; then
  # OpenTX on Sky9x
  rm -rf *
  cmake ${COMMON_OPTIONS} -DPCB=SKY9X -DHELI=YES DLUA=YES -DMULTIMODULE=YES -DTELEMETRY=FRSKY -DPPM_LIMITS_SYMETRICAL=YES -DVARIO=YES -DGAUGES=YES -DAUTOSWITCH=YES -DAUTOSOURCE=YES -DAUDIO=YES -DGPS=YES -DPPM_CENTER_ADJUSTABLE=YES -DFLIGHT_MODES=YES -DOVERRIDE_CHANNEL_FUNCTION=YES -DFRSKY_STICKS=YES ${SRCDIR}
  make -j${CORES} ${FIRMARE_TARGET}
  make -j${CORES} simu
  make -j${CORES} gtests ; ./gtests ${TEST_OPTIONS}
fi
