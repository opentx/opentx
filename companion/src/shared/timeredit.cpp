...
void TimerEdit::setTime(int seconds)
{
  ...
  //qDebug() << seconds << qrH.quot << qrH.rem << qrM.quot << qrM.rem << val << text();
...
void TimerEdit::textEditedHandler()
{
  ...
  //qDebug() << text() << sec << getTimeSeconds() << validator()->validate(text(), sec);
}
