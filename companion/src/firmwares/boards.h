...
inline bool IS_FAMILY_HORUS_T16(Board::Type board)
{
...
{
  return IS_FAMILY_HORUS_T16(board) || IS_TARANIS(board);
}

inline bool IS_STM32(Board::Type board)
{
  return IS_TARANIS(board) || IS_FAMILY_HORUS_T16(board);
}
...
{
  return IS_FAMILY_HORUS_T16(board) || IS_TARANIS_X9(board);
}
...
inline bool IS_ACCESS_RADIO(Board::Type board, QString &id)
{
  return (IS_TARANIS_XLITES(board) || IS_TARANIS_X9LITE(board) || board == Board::BOARD_TARANIS_X9DP_2019 || board == Board::BOARD_X10_EXPRESS || IS_TARANIS_X7_ACCESS(board) ||
          (IS_FAMILY_HORUS_T16(board) && id.contains("internalaccess")));
}
