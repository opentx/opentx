...
RawSource RawSource::convert(RadioDataConversionState & cstate)
{
  ...
  if (type == SOURCE_TYPE_STICK) {
    ...
    if (IS_TARANIS(cstate.toType) && IS_FAMILLY_HORUS_T16(cstate.fromType)) {
      if (index == 6)
        index = 5;  // pot S2 to S2
      else if (index == 5)
        index = -1;  //  6P on Horus doesn't exist on Taranis
    }
    else  if (IS_FAMILLY_HORUS_T16(cstate.toType) && IS_TARANIS(cstate.fromType) && index == 5)
    {
  ...
  if (type == SOURCE_TYPE_SWITCH) {
    ...
    if (IS_TARANIS_X7(cstate.toType) && (IS_TARANIS_X9(cstate.fromType) || IS_FAMILLY_HORUS_T16(cstate.fromType))) {
    ...
    else if (IS_JUMPER_T12(cstate.toType) && (IS_TARANIS_X9(cstate.fromType) || IS_FAMILLY_HORUS_T16(cstate.fromType))) {
    ...
    else if ((IS_TARANIS_X9(cstate.toType) || IS_FAMILLY_HORUS_T16(cstate.toType)) && IS_TARANIS_X7(cstate.fromType)) {
      if (index == 4) {
        index = 5;  // SF to SF
      }
      else if (index == 5) {
    ...
    else if ((IS_TARANIS_X9(cstate.toType) || IS_FAMILLY_HORUS_T16(cstate.toType)) && IS_JUMPER_T12(cstate.fromType)) {
      if (index == 4) {
        index = 5;  // SF to SF
      }
      else if (index == 5) {
