...
enum SpanishPrompts {
  ...
  PROMPT_CIEN = PROMPT_NUMBERS_BASE+100,
  PROMPT_CIENTO = PROMPT_NUMBERS_BASE+101,
  PROMPT_DOSCIENTOS = PROMPT_NUMBERS_BASE+102,
  PROMPT_TRESCIENTOS,
  PROMPT_CUATROCIENTOS,
  PROMPT_QUINIENTOS,
  PROMPT_SESCIENTOS,
  PROMPT_SETECIENTOS,
  PROMPT_OCHOCIENTOS,
...
*/
  ...
  if (number >= 100) {
    PUSH_NUMBER_PROMPT(PROMPT_CIENTO + number/100);
    number %= 100;
    if (number == 0)
      number = -1;
  }
...
PLAY_FUNCTION(playDuration, int16_t seconds)
{
  ...
  if (tmp > 0) {
    ...
    if (tmp > 1) {
      PLAY_NUMBER(tmp, 0, 0);
      PUSH_NUMBER_PROMPT(PROMPT_HORAS);
    } else {
      PUSH_NUMBER_PROMPT(PROMPT_UNA);
      PUSH_NUMBER_PROMPT(PROMPT_HORA);
    }
  }
  ...
  if (tmp > 0 || ore >0) {
    if (tmp != 1) {
      PLAY_NUMBER(tmp, 0, 0);
      PUSH_NUMBER_PROMPT(PROMPT_MINUTOS);
    } else {
      PUSH_NUMBER_PROMPT(PROMPT_UN);
      PUSH_NUMBER_PROMPT(PROMPT_MINUTO);
    }
    PUSH_NUMBER_PROMPT(PROMPT_Y);
  }

  if (seconds != 1) {
    PLAY_NUMBER(seconds, 0, 0);
    PUSH_NUMBER_PROMPT(PROMPT_SEGUNDOS);
  } else {
    PUSH_NUMBER_PROMPT(PROMPT_UN);
    PUSH_NUMBER_PROMPT(PROMPT_SEGUNDO);
  }
}
