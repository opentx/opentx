    ...
    for (uint8_t j=0; j<5; j++) {
      ...
      switch (j) {
        case ITEM_CUSTOM_FUNCTIONS_SWITCH:
          ...
          if (active || AUTOSWITCH_ENTER_LONG()) CHECK_INCDEC_SWITCH(event, CFN_SWITCH(cfn), SWSRC_FIRST, functions == g_model.customFn ? SWSRC_TELEMETRY_STREAMING : SWSRC_LAST, eeFlags, isSwitchAvailableInCustomFunctions);
