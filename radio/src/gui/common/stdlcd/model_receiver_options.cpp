...
bool isFPort2Available(int portType)
{
  uint8_t receiverId = reusableBuffer.hardwareAndSettings.receiverSettings.receiverId;
  if (!IS_RECEIVER_CAPABILITY_ENABLED(RECEIVER_CAPABILITY_FPORT2) && portType == 2)
  {
    return false;
  }
  return true;
...
void menuModelReceiverOptions(event_t event)
{
  ...
  SUBMENU_NOTITLE(ITEM_RECEIVER_SETTINGS_PINMAP_FIRST + outputsCount, {
    ...
    IF_RECEIVER_CAPABILITY(RECEIVER_CAPABILITY_FPORT, 0) || IF_RECEIVER_CAPABILITY(RECEIVER_CAPABILITY_FPORT2, 0),
