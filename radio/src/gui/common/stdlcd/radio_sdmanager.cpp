...
void menuRadioSdManagerInfo(event_t event)
{
  ...
  lcdDrawChar(lcdRightPos, 3*FH, 'M');
  ...
  lcdDrawChar(lcdRightPos, 4*FH, '/');
  lcdDrawNumber(lcdRightPos+FW, 4*FH, sdGetNoSectors()/1000, LEFT);
#else
  lcdDrawNumber(10*FW, 4*FH, sdGetNoSectors()/1000, LEFT);
#endif
  lcdDrawChar(lcdRightPos, 4*FH, 'k');

  lcdDrawTextAlignedLeft(5*FH, STR_SD_SPEED);
  lcdDrawNumber(10*FW, 5*FH, SD_GET_SPEED()/1000, LEFT);
  lcdDrawText(lcdRightPos, 5*FH, "kb/s");
}
