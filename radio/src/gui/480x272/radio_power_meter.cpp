...
constexpr coord_t POWER_METER_2ND_COLUMN = 100;
...
bool menuRadioPowerMeter(event_t event)
{
  ...
  if (reusableBuffer.powerMeter.attn != 0) {
    lcdDrawCenteredText(MENU_BODY_TOP, STR_POWERMETER_ATTN_NEEDED, BOLD | INVERS);
  }

  for (uint8_t i=0; i<POWER_METER_FIELDS_MAX; i++) {
    LcdFlags attr = (menuVerticalPosition == i ? (s_editMode > 0 ? INVERS | BLINK : INVERS) : 0);
    coord_t y = MENU_BODY_TOP + 2 * FH + i * FH;

    switch (i) {
      case POWER_METER_FREQ_RANGE:
        lcdDrawText(MENUS_MARGIN_LEFT, y, STR_POWERMETER_FREQ);
        lcdDrawNumber(POWER_METER_2ND_COLUMN, y, reusableBuffer.powerMeter.freq / 1000000, LEFT | attr);
      ...
      case POWER_METER_ATTENUATOR:
        ...
        lcdDrawNumber(POWER_METER_2ND_COLUMN, y, -10 * reusableBuffer.powerMeter.attn, LEFT | attr);
      ...
      case POWER_METER_MAX:
        ...
        lcdDrawNumber(POWER_METER_2ND_COLUMN, y, -10 + 10 * reusableBuffer.powerMeter.attn);
        lcdDrawText(lcdNextPos + 3, y, "dBm/ ");
        drawPower(lcdNextPos + 3, y, -10 + 10 * reusableBuffer.powerMeter.attn, 0);
        if (attr) {
          reusableBuffer.powerMeter.attn = checkIncDec(event, reusableBuffer.powerMeter.attn, 0, 5, 0);
        }
        break;
      ...
      case POWER_METER_POWER:
        ...
        if (reusableBuffer.powerMeter.power) {
          lcdDrawNumber(POWER_METER_2ND_COLUMN, y, reusableBuffer.powerMeter.power + reusableBuffer.powerMeter.attn * 1000, LEFT | PREC2);
      ...
      case POWER_METER_PEAK:
        ...
        if (reusableBuffer.powerMeter.peak) {
          lcdDrawNumber(POWER_METER_2ND_COLUMN, y, reusableBuffer.powerMeter.peak + reusableBuffer.powerMeter.attn * 1000, LEFT | PREC2);
          lcdDrawText(lcdNextPos, y, "dBm");
        }
        break;
    }
