...
bool menuTextView(event_t event)
{
  ...
  switch (event) {
    ...
    case EVT_ROTARY_RIGHT:
      if (menuVerticalOffset + TEXT_VIEWVER_LINES >= lines_count)
        break;
      else
        ++menuVerticalOffset;
      sdReadTextFile(reusableBuffer.viewText.filename, reusableBuffer.viewText.lines, lines_count);
  ...
  for (int i=0; i<TEXT_VIEWVER_LINES; i++) {
  ...
  drawVerticalScrollbar(LCD_W-5, 50, 195, menuVerticalOffset, lines_count, TEXT_VIEWVER_LINES);
