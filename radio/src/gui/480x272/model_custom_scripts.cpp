...
bool menuModelCustomScriptOne(event_t event)
{
  ...
  for (int k=0; k<NUM_BODY_LINES; k++) {
    ...
    if (i == ITEM_MODEL_CUSTOMSCRIPT_FILE) {
      ...
      if (scriptInputsOutputs[s_currIdx].inputs[inputIdx].type == 0) {
        ...
        drawMixerSource(SCRIPT_ONE_2ND_COLUMN_POS, y, *source + scriptInputsOutputs[s_currIdx].inputs[inputIdx].def, attr);
        if (attr) {
  ...
  if (scriptInputsOutputs[s_currIdx].outputsCount > 0) {
    ...
    for (int i=0; i<scriptInputsOutputs[s_currIdx].outputsCount; i++) {
      drawMixerSource(SCRIPT_ONE_3RD_COLUMN_POS+INDENT_WIDTH, MENU_CONTENT_TOP+i*FH, MIXSRC_FIRST_LUA+(s_currIdx*MAX_SCRIPT_OUTPUTS)+i, 0);
