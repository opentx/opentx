...
void menuCommonCalib(event_t event)
{
  ...
  switch (reusableBuffer.calib.state) {
    case CALIB_START:
      // START CALIBRATION
      if (!READ_ONLY()) {
        lcdDrawTextAlignedLeft(MENU_HEADER_HEIGHT+FH+2, STR_MENUTOSTART);
      }
      break;

    case CALIB_SET_MIDPOINT:
      // SET MIDPOINT
      lcdDrawText(0*FW, MENU_HEADER_HEIGHT+2, STR_SETMIDPOINT, INVERS);
      lcdDrawTextAlignedLeft(MENU_HEADER_HEIGHT+FH+2, STR_MENUWHENDONE);
    ...
    case CALIB_MOVE_STICKS:
      ...
      lcdDrawText(0*FW, MENU_HEADER_HEIGHT+2, STR_MOVESTICKSPOTS, INVERS);
      lcdDrawTextAlignedLeft(MENU_HEADER_HEIGHT+FH+2, STR_MENUWHENDONE);
  ...
  lcdDrawTextAlignedLeft(LCD_H-FH, STR_FACTORYRESET);
  lcdDrawFilledRect(0, LCD_H-FH-1, LCD_W, FH+1, SOLID);
