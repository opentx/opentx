    ...
    switch (i) {
      ...
      case LS_FIELD_V1:
      {
        ...
        if (cstate == LS_FAMILY_BOOL || cstate == LS_FAMILY_STICKY || cstate == LS_FAMILY_EDGE) {
          drawSwitch(CSWONE_2ND_COLUMN, y, v1_val, attr | ((v1_val && getSwitch(v1_val)) ? BOLD : 0));
      ...
      case LS_FIELD_V2:
      {
        ...
        if (cstate == LS_FAMILY_BOOL || cstate == LS_FAMILY_STICKY) {
          drawSwitch(CSWONE_2ND_COLUMN, y, cs->v2, attr | ((cs->v2 && getSwitch(cs->v2)) ? BOLD : 0));
      ...
      case LS_FIELD_ANDSW:
        ...
        drawSwitch(CSWONE_2ND_COLUMN, y, cs->andsw, attr | ((cs->andsw && getSwitch(cs->andsw)) ? BOLD : 0));
        if (attr) {
...
void menuModelLogicalSwitches(event_t event)
{
  ...
  for (uint8_t i=0; i<LCD_LINES-1; i++) {
    ...
    if (cs->func > 0) {
      ...
      if (cstate == LS_FAMILY_BOOL || cstate == LS_FAMILY_STICKY) {
        drawSwitch(CSW_2ND_COLUMN, y, cs->v1, (cs->v1 && getSwitch(cs->v1)) ? BOLD : 0);
        drawSwitch(CSW_3RD_COLUMN, y, cs->v2, (cs->v2 && getSwitch(cs->v2)) ? BOLD : 0);
      }
      else if (cstate == LS_FAMILY_COMP) {
      ...
      // CSW and switch
      drawSwitch(CSW_4TH_COLUMN, y, cs->andsw, (cs->andsw && getSwitch(cs->andsw)) ? BOLD : 0);
    }
  }
}
