...
{
  GVarData * gvar = &g_model.gvars[s_currIdxGvar];

  drawStringWithIndex(PSIZE(TR_GVARS)*FW+FW, 0, STR_GV, s_currIdxGvar+1, 0);
  drawGVarValue(32*FW, 0, s_currIdxGvar, getGVarValue(s_currIdxGvar, getFlightMode()));
  ...
  for (int i=0; i<NUM_BODY_LINES; i++) {
    ...
    switch (k) {
      ...
      case GVAR_FIELD_MIN:
        ...
        drawGVarValue(GVAR_2ND_COLUMN, y, s_currIdxGvar, GVAR_MIN+gvar->min, LEFT|attr);
        if (attr) gvar->min = checkIncDec(event, GVAR_MIN+gvar->min, GVAR_MIN, GVAR_MAX-gvar->max, EE_MODEL) - GVAR_MIN;
        break;

      case GVAR_FIELD_MAX:
        lcdDrawText(0, y, STR_MAX);
        drawGVarValue(GVAR_2ND_COLUMN, y, s_currIdxGvar, GVAR_MAX-gvar->max, LEFT|attr);
      ...
      default:
        ...
        editGVarValue(GVAR_2ND_COLUMN, y, event, s_currIdxGvar, k-GVAR_FIELD_FM0, LEFT|attr);
        break;
    }
  }
}
