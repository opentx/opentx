  ...
  for (uint8_t i=0; i<NUM_BODY_LINES; ++i) {
    ...
    switch (k) {
      ...
      case ITEM_MODEL_EXTERNAL_MODULE_CHANNELS:
      {
        ...
        if ((int8_t)PORT_CHANNELS_ROWS(moduleIdx) >= 0) {
          ...
          lcdDrawNumber(lcdRightPos, y, moduleData.channelsStart+1, LEFT | (menuHorizontalPosition==0 ? attr : 0));
          lcdDrawChar(lcdRightPos, y, '-');
          lcdDrawNumber(lcdRightPos + FW+1, y, moduleData.channelsStart+NUM_CHANNELS(moduleIdx), LEFT | (menuHorizontalPosition==1 ? attr : 0));
          if (attr && (editMode>0 || p1valdiff)) {
            switch (menuHorizontalPosition) {
              case 0:
    ...
    case ITEM_MODEL_EXTERNAL_MODULE_STATUS: {
      ...
      case ITEM_MODEL_PPM2_PROTOCOL:
        ...
        lcdDrawNumber(lcdRightPos, y, g_model.moduleData[1].channelsStart+1, LEFT | (menuHorizontalPosition<=0 ? attr : 0));
        lcdDrawChar(lcdRightPos, y, '-');
        lcdDrawNumber(lcdRightPos + FW+1, y, g_model.moduleData[1].channelsStart+8+g_model.moduleData[1].channelsCount, LEFT | (menuHorizontalPosition!=0 ? attr : 0));
        if (attr && (editMode>0 || p1valdiff)) {
          switch (menuHorizontalPosition) {
            case 0:
