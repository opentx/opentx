...
void menuModelTelemetryFrsky(event_t event)
{
  ...
  for (uint8_t i=0; i<LCD_LINES-1; i++) {
    ...
    if (k>=ITEM_TELEMETRY_SENSOR1 && k<ITEM_TELEMETRY_SENSOR1+MAX_TELEMETRY_SENSORS) {
      ...
      lcdDrawChar(lcdRightPos, y, ':', attr);
      ...
      if (telemetryItem.isAvailable()) {
        ...
        if (isOld) lcdDrawChar(lcdRightPos, y, ']');
      }
      else {
    ...
    switch (k) {
      ...
      case ITEM_TELEMETRY_A2_RANGE:
        ...
        lcdDrawTextAtIndex(lcdRightPos, y, STR_VTELEMUNIT, channel.type, menuHorizontalPosition!=0 ? attr : 0);
