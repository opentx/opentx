...
void drawGPSCoord(uint8_t y, char direction, int16_t bp, int16_t ap)
{
  if (telemetryData.hub.gpsFix >= 0) {
    ...
    lcdDrawChar(lcdRightPos, y, '@');
    uint8_t mn = bp % 100; // TODO div_t
    if (g_eeGeneral.gpsFormat == 0) {
      lcdDrawChar(lcdRightPos+FWNUM, y, direction);
      lcdDrawNumber(lcdRightPos+FW+FW+1, y, mn, LEFT|LEADING0, 2); // mm before '.'
      lcdDrawSolidVerticalLine(lcdRightPos, y, 2);
      uint16_t ss = ap * 6;
      lcdDrawNumber(lcdRightPos+3, y, ss / 1000, LEFT|LEADING0, 2); // ''
      lcdDrawPoint(lcdRightPos, y+FH-2, 0); // small decimal point
      lcdDrawNumber(lcdRightPos+2, y, ss % 1000, LEFT|LEADING0, 3); // ''
      lcdDrawSolidVerticalLine(lcdRightPos, y, 2);
      lcdDrawSolidVerticalLine(lcdRightPos+2, y, 2);
    }
    else {
      lcdDrawNumber(lcdRightPos+FW, y, mn, LEFT|LEADING0, 2); // mm before '.'
      lcdDrawPoint(lcdRightPos, y+FH-2, 0); // small decimal point
      lcdDrawNumber(lcdRightPos+2, y, ap, LEFT|UNSIGN|LEADING0, 4); // after '.'
      lcdDrawChar(lcdRightPos+1, y, direction);
    }
  }
  else {
    // no fix
