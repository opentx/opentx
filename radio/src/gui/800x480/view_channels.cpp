...
#define X_MARGIN                       ((800-X_OFFSET*2-COLUMN_SIZE*4)/3)
#define Y_MARGIN                       3
...
bool menuChannelsMonitor(event_t event, uint8_t page)
{
  ...
  x = X_OFFSET;
  for (uint8_t i = 0; i < 32; i++, channel++, y += (ROW_HEIGHT + Y_MARGIN)) {
    if (i == 8){
        x = X_OFFSET + COLUMN_SIZE + X_MARGIN;
        y = Y_OFFSET;}
        else if(i == 16){
          x = X_OFFSET + COLUMN_SIZE * 2 +X_MARGIN * 2;
          y = Y_OFFSET;}
          else if(i == 24){
            x = X_OFFSET + COLUMN_SIZE * 3 + X_MARGIN * 3;
            y = Y_OFFSET;}
