...
class Layout2x4: public Layout
{
  public:
    ...
    virtual void create()
    {
      ...
      persistentData->options[6].boolValue = true;
    ...
    {
      Zone zone;
      zone.x = (index >= 4) ? 410 : 60;
      zone.y = 60 + (index % 4) * 95;
      zone.w = 330;
      zone.h = 75;
      return zone;
    }
...
void Layout2x4::refresh()
{
  ...
  if (persistentData->options[4].boolValue) {
    ...
    lcdDrawSolidFilledRect(55, 55, 340, 370, CUSTOM_COLOR);
  }

  if (persistentData->options[6].boolValue) {
    lcdSetColor(persistentData->options[7].unsignedValue);
    lcdDrawSolidFilledRect(405, 55, 340, 370, CUSTOM_COLOR);
  }
