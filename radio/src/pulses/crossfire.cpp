...
#define CROSSFIRE_CENTER            0x3E0
  ...
  #define CROSSFIRE_CH_CENTER(ch)            (CROSSFIRE_CENTER + ((limitAddress(ch)->ppmCenter * 4) / 5))
#else
  #define CROSSFIRE_CH_CENTER(ch)            (CROSSFIRE_CENTER)
#endif
...
uint8_t createCrossfireChannelsFrame(uint8_t * frame, int16_t * pulses)
{
  ...
  *buf++ = CHANNELS_ID;
  ...
  for (int i=0; i<CROSSFIRE_CHANNELS_COUNT; i++) {
    uint32_t val = limit(0, CROSSFIRE_CH_CENTER(i) + (((pulses[i]) * 4) / 5), 2*CROSSFIRE_CENTER);
