...
void setupPulsesMultimodule(unsigned int port)
{
  ...
  *modulePulsesData[EXTERNAL_MODULE].dsm2.ptr++ = modulePulsesData[EXTERNAL_MODULE].dsm2.value;
  ...
  int type = g_model.moduleData[port].multi_rfProtocol +1;
  ...
  if (g_model.moduleData[port].multi_rfProtocol == MM_RF_PROTO_FRSKY) {
    if(subtype == 1)
      type = 3;
     else
      type = 15;
  ...
  // byte 3
  sendByteMulti(g_model.moduleData[port].optionValue);
