...
// this version for traditional scaling
...
// Range for pulses (channels output) is [-1024:+1024]
// this version for new 'raw' 12-bit scaling
uint8_t createGhostChannelsFrame12BitRaw(uint8_t * frame, int16_t * pulses)
{
  static uint8_t lastGhostFrameId = GHST_UL_RC_CHANS_HS4_12_5TO8;
  uint8_t ghostUpper4Offset = 0;
  ...
  *buf++ = lastGhostFrameId;
  ...
  uint8_t bitsavailable = 0;
  for (int i = 0; i < 4; i++) {
    uint32_t value = limit(0, (1024 + (pulses[i] + 2 * PPM_CH_CENTER(i) - 2 * PPM_CENTER)) << 1, 0xFFF);
    bits |= value << bitsavailable;
    bitsavailable += GHST_CH_BITS_12;
    while (bitsavailable >= 8) {
      *buf++ = bits;
  ...
  for (int i = 4; i < 8; ++i) {
    uint8_t channelIndex = i + ghostUpper4Offset;
    *buf++ = limit(0, 128 + ((pulses[channelIndex] + 2 * PPM_CH_CENTER(channelIndex) - 2 * PPM_CENTER) >> 3), 0xFF);
  }
  ...
  *buf++ = crc8(crc_start, GHST_UL_RC_CHANS_SIZE - 1);
  ...
  switch (lastGhostFrameId) {
    case GHST_UL_RC_CHANS_HS4_12_5TO8:
      lastGhostFrameId = GHST_UL_RC_CHANS_HS4_12_9TO12;
      break;
    case GHST_UL_RC_CHANS_HS4_12_9TO12:
      lastGhostFrameId = GHST_UL_RC_CHANS_HS4_12_13TO16;
      break;
    case GHST_UL_RC_CHANS_HS4_12_13TO16:
      lastGhostFrameId = GHST_UL_RC_CHANS_HS4_12_5TO8;
      break;
  }
...
void setupPulsesGhost()
{
  if (telemetryProtocol == PROTOCOL_TELEMETRY_GHOST) {
    ...
    auto *ghost = &extmodulePulsesData.ghost;

    if (LUA_DEFINED() && outputTelemetryBuffer.destination == TELEMETRY_ENDPOINT_SPORT) {
      memcpy(ghost->pulses, outputTelemetryBuffer.data, outputTelemetryBuffer.size);
      ghost->length = outputTelemetryBuffer.size;
      outputTelemetryBuffer.reset();
    }
    else if (moduleState[EXTERNAL_MODULE].counter == GHST_MENU_CONTROL) {
      ghost->length = createGhostMenuControlFrame(ghost->pulses, &channelOutputs[g_model.moduleData[EXTERNAL_MODULE].channelsStart]);
    }
    else {
      if (module.ghost.raw12bits)
        ghost->length = createGhostChannelsFrame12BitRaw(ghost->pulses, &channelOutputs[g_model.moduleData[EXTERNAL_MODULE].channelsStart]);
      else
        ghost->length = createGhostChannelsFrame(ghost->pulses, &channelOutputs[g_model.moduleData[EXTERNAL_MODULE].channelsStart]);
    }
