...
/* result is lua table containing members ["lat"] and ["lon"] as lua_Number (doubles) in decimal degrees */
{
  ...
  lua_pushtablenumber(L, "lat", telemetryItem.gps.latitude / 1000000.0f);
  lua_pushtablenumber(L, "pilot-lat", telemetryItem.pilotLatitude / 1000000.0f);
  lua_pushtablenumber(L, "lon", telemetryItem.gps.longitude / 1000000.0f);
  lua_pushtablenumber(L, "pilot-lon", telemetryItem.pilotLongitude / 1000000.0f);
}
...
static void luaPushCells(lua_State* L, TelemetrySensor & telemetrySensor, TelemetryItem & telemetryItem)
{
  if (telemetryItem.cells.count == 0)
    ...
    for (int i = 0; i < telemetryItem.cells.count; i++) {
      ...
      lua_pushnumber(L, telemetryItem.cells.values[i].value / 100.0f);
      lua_settable(L, -3);
    }
  }
}
...
void luaGetValueAndPush(lua_State* L, int src)
{
  ...
  if (src >= MIXSRC_FIRST_TELEM && src <= MIXSRC_LAST_TELEM) {
    ...
    lua_pushnumber(L, float(value)/10.0f);
  }
  else {
    lua_pushinteger(L, value);
  }
...
{
  lua_newtable(L);
  lua_pushtablenumber(L, "battMin", (90+g_eeGeneral.vBatMin)/10.0f);
  lua_pushtablenumber(L, "battMax", (120+g_eeGeneral.vBatMax)/10.0f);
