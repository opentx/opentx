...
/*!
   ...
   \param delay Number of ms to sleep between loops (not counted in statistics). Default is zero (0).
*/
void cliBenchIterate(const char * title, int iter, void (*cb)(int), const std::vector<int> & args, const std::vector<const char *> & names, uint32_t delay = 0)
{
  ...
  RTOS_SET_TASK_PRIORITY(cliTaskId, 1);  // prevent thread hijack
  ...
  RTOS_SET_TASK_PRIORITY(cliTaskId, CLI_TASK_PRIO);
  if (args.size() < 2)
    return;
