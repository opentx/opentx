...
void convertModelData_218_to_219(ModelData &model)
{
  ...
  for (uint8_t i=0; i<MAX_TELEMETRY_SENSORS_218; i++) {
    ...
    if (oldModel.telemetrySensors[i].type == 0 && (oldModel.moduleData[0].type ==  MODULE_TYPE_XJT_PXX1 || oldModel.moduleData[1].type == MODULE_TYPE_XJT_PXX1) && ZLEN(oldModel.telemetrySensors[i].label) > 0)
