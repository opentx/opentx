  ...
  switch (sensor.formula) {
    ...
    case TELEM_FORMULA_DIST:
      if (sensor.dist.gps) {
        ...
        uint32_t dist = (uint32_t )(((uint64_t)EARTH_RADIUS * angle) / 1000000);  // radius * angle will overflow uint32_t in some case
        uint32_t result = dist*dist;

        angle = abs(gpsItem.gps.longitude - gpsItem.pilotLongitude);
        dist = (uint32_t )(((uint64_t)gpsItem.distFromEarthAxis * angle) / 1000000);
        result += dist*dist;
