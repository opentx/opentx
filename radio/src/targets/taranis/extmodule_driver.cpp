...
void extmodulePpmStart()
{
  ...
  // TODO need to adapt, this will break all other boards
  EXTMODULE_TIMER->CR1 &= ~TIM_CR1_CEN;
  ...
  EXTMODULE_TIMER->CCMR1 = TIM_CCMR1_OC2M_1 | TIM_CCMR1_OC2M_2; // PWM mode 1
  EXTMODULE_TIMER->CCMR2 = TIM_CCMR2_OC3PE;
  EXTMODULE_TIMER->SR &= ~TIM_SR_CC3IF; // Clear flag
  EXTMODULE_TIMER->CCR3 = 40000; // The first frame will be sent in 20ms
  EXTMODULE_TIMER->DIER |= TIM_DIER_CC3IE; // Enable this interrupt
  EXTMODULE_TIMER->CR1 |= TIM_CR1_CEN;
...
void extmoduleSendNextFrame()
{
  switch(moduleState[EXTERNAL_MODULE].protocol) {
    case PROTOCOL_CHANNELS_PPM:
      EXTMODULE_TIMER->CCR2 = GET_MODULE_PPM_DELAY(EXTERNAL_MODULE) * 2;
      EXTMODULE_TIMER->CCER = EXTMODULE_TIMER_OUTPUT_ENABLE | (GET_MODULE_PPM_POLARITY(EXTERNAL_MODULE) ? EXTMODULE_TIMER_OUTPUT_POLARITY : 0); //     // we are using complementary output so logic has to be reversed here
      EXTMODULE_TIMER->CCR3= *(extmodulePulsesData.ppm.ptr - 1) - 4000; // 2mS in advance
      EXTMODULE_TIMER_DMA_STREAM->CR &= ~DMA_SxCR_EN; // Disable DMA
...
{
  EXTMODULE_TIMER->DIER &= ~TIM_DIER_CC3IE; // Stop this interrupt
  EXTMODULE_TIMER->SR &= ~TIM_SR_CC3IF;
