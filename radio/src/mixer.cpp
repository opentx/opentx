...
void evalMixes(uint8_t tick10ms)
{
    ...
    for (uint8_t p=0; p<MAX_FLIGHT_MODES; p++) {
      if (flightModesFade & (0x01 << p)) {
        ...
        for (uint8_t i=0; i<MAX_OUTPUT_CHANNELS; i++)
          sum_chans512[i] += (chans[i] >> (__builtin_clz(abs(chans[i])) >= 17 ? 0 : 17 - __builtin_clz(abs(chans[i])))) * fp_act[p];
        weight += fp_act[p];
      }
    }
    assert(weight);
  ...
  for (uint8_t i=0; i<MAX_OUTPUT_CHANNELS; i++) {
    ...
    int32_t q = (flightModesFade ? ((sum_chans512[i] / weight) << (__builtin_clz(abs(chans[i])) >= 17 ? 0 : 17 - __builtin_clz(abs(chans[i])))) : chans[i]);
