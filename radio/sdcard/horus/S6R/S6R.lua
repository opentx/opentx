...
local function drawProgressBar()
  local width = (380 * refreshIndex) / #fields
  lcd.drawRectangle(100, 10, 144, 6)
...
local wingBitmaps = { Bitmap.open("plane.bmp"), Bitmap.open("delta.bmp"), Bitmap.open("vtail.bmp") }
local mountBitmaps = { Bitmap.open("horz.bmp"), Bitmap.open("horz-r.bmp"), Bitmap.open("vert.bmp"), Bitmap.open("vert-r.bmp") }

local function runConfigPage(event)
  fields = configFields
  local result = runFieldsPage(event)
  if fields[1][4] ~= nil then
    lcd.drawBitmap(wingBitmaps[1 + fields[1][4]], 20, 28)
  end
  if fields[2][4] ~= nil then
    lcd.drawBitmap(mountBitmaps[1 + fields[2][4]], 128, 28)
  end
  return result
end
...
local function runCalibrationPage(event)
  ...
  lcd.drawBitmap(Bitmap.open(position .. ".bmp"), 200, 90, 200)
  for index = 1, 3, 1 do
    local field = fields[index]
    lcd.drawText(80, 50+20*index, field[1], TEXT_COLOR)
    lcd.drawNumber(90, 50+20*index, field[4]/10, LEFT+PREC2)
  end
...
local function run(event)
  if event == nil then
    ...
    return 2
  elseif event == EVT_PAGE_BREAK or event == EVT_PAGEUP_FIRST  then
    selectPage(1)
  elseif event == EVT_PAGE_LONG or event == EVT_PAGEDN_FIRST then
    killEvents(event);
    selectPage(-1)
  end
