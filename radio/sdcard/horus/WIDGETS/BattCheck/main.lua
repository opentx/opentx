...
local function calculateBatteryData(wgt)
  ...
  if wgt.options.LowestCell == 1 then
    _mainValue = wgt.cellMin
  elseif wgt.options.LowestCell == 0 then
    _mainValue = wgt.cellSum
  else
    _mainValue = "-1"
  end
  ...
  -- SecondaryValue
  if wgt.options.LowestCell == 1 then
    _secondaryValue = wgt.cellSum
  elseif wgt.options.LowestCell == 0 then
    _secondaryValue = wgt.cellMin
  else
    _secondaryValue = "-1"
  end
...
local function refreshZoneSmall(wgt)
  ...
  local topLine = string.format("%2.1fV      %2.0f%%", _mainValue, wgt.cellPercent)
...
local function refreshZoneMedium(wgt)
  ...
  -- draw values
  --lcd.drawText(wgt.zone.x + wgt.zone.w, wgt.zone.y, string.format("%2.1fV", _mainValue), DBLSIZE + CUSTOM_COLOR + RIGHT + wgt.shadowed + wgt.no_telem_blink)
  lcd.drawText(wgt.zone.x, wgt.zone.y + 35, string.format("%2.1fV", _mainValue), DBLSIZE + CUSTOM_COLOR + wgt.shadowed + wgt.no_telem_blink)
  ...
  for i = 1, wgt.cellCount, 1 do
    ...
    lcd.setColor(CUSTOM_COLOR, WHITE)
    lcd.drawText(wgt.zone.x + pos[i].x + 10, wgt.zone.y + pos[i].y, string.format("%.2f", wgt.cellDataLive[i]), SMLSIZE + CUSTOM_COLOR + wgt.shadowed + wgt.no_telem_blink)
...
local function refreshZoneLarge(wgt)
  ...
  lcd.drawText(wgt.zone.x + wgt.zone.w, wgt.zone.y + 30, string.format("%2.1fV", _mainValue), RIGHT + DBLSIZE + CUSTOM_COLOR + wgt.shadowed)
  lcd.drawText(wgt.zone.x + wgt.zone.w, wgt.zone.y + 70, string.format("%2.1fV %2.1fS", _secondaryValue, wgt.cellCount), RIGHT + SMLSIZE + CUSTOM_COLOR + wgt.shadowed)
  -- fill batt
  lcd.setColor(CUSTOM_COLOR, getPercentColor(wgt.cellPercent))
...
local function refreshZoneXLarge(wgt)
  ...
  lcd.drawText(wgt.zone.x + wgt.zone.w, wgt.zone.y + myBatt.y + 30, string.format("%2.1fV", _mainValue), RIGHT + DBLSIZE + CUSTOM_COLOR + wgt.shadowed + wgt.no_telem_blink)
  lcd.drawText(wgt.zone.x + wgt.zone.w, wgt.zone.y + myBatt.y + 105, string.format("%2.1fV %2.1fS", _secondaryValue, wgt.cellCount), RIGHT + SMLSIZE + CUSTOM_COLOR + wgt.shadowed + wgt.no_telem_blink)
