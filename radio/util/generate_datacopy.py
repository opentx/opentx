...
def check_translation_unit_errors(diags, prefix=''):
    for diag in diags:
        if not "stdbool.h" in str(diag):
            print(prefix + str(diag), file=sys.stderr)
            check_translation_unit_errors(diag.children, '  ' + prefix)
            return False
    return True
...
def main():
    ...
    if not check_translation_unit_errors(translation_unit.diagnostics):
        sys.exit(-1)
